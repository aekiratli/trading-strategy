...
async def pmax_bbands(parity, state, file_name, logger, zone, lowerband, pmax, close):
    ...
    if parity["pmax_bbands"] == True and parity["pmax"] == True and parity["bbands"] == True:
        ...
        if zone == "up" and lowerband >= pmax and state["pmax_bbands_has_ordered"] == False:
            ...
            await telegram_bot_sendtext(f"* {parity['symbol']}-{parity['interval']} - PMAX-BBANDS - LIMIT BUY ORDER* Buy Price = {buy_price}, Amount = {amount}%0A%0A *{parity['symbol']}-{parity['interval']} - PMAX-BBANDS - LIMIT SELL ORDER* Sell Price = {sell_price}, Amount = {amount}", True)
